...
module.exports = class BasestoreSession {
  constructor (client, sessionState, basestorevault) {
    this._client = client
    this._corestore = basestorevault
    this._sessionState = sessionState

    const feedListener = (feed) => {
      this._client.basestorevault.onFeedNoReply({
        key: feed.key
      })
    }
  ...
  async open ({ id, key, name, weak }) {
    ...
    const base = this._corestore.get({ key, _name: name, default: !!name })
