...
module.exports = class NetworkSession {
  constructor (client, sessionState, basestorevault, networker, db, networkStates, opts = {}) {
    this._client = client
    this._sessionState = sessionState
    this._networkStates = networkStates
    this._corestore = basestorevault
  ...
  _setConfiguringTimeouts (discoveryKey) {
    // technically the base can load *during* the configuration call as well
    // in practice that wont imply the pipelining guarantee so we dont need
    // to worry about it
    if (!this._corestore.isLoaded({ discoveryKey })) return
    const base = this._corestore.get({ discoveryKey })
    const cbSet = new Set()
    const mainTimeouts = base.timeouts
    let called = false
    base.timeouts = {
      get: (cb) => {
